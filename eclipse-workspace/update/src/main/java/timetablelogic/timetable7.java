...
public class timetable7 extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException{
		...
		String htmlContent = "<!DOCTYPE html>"
		        ...
		        + "        h1 {"
		        + "            font-size: 32px;"
		        + "            margin-bottom: 50px; /* Increase padding below the title */"
		        + "            text-align: center;"
		        + "        }"
		        + "        h2 {"
		        + "            font-size: 24px; /* Separate styling for h2 */"
		        + "            margin-bottom: 20px; /* Separate margin for h2 */"
		        + "            text-align: left; /* Left-align the h2 tag */"
		        + "        }"
		        + "        p {"
		        + "            font-size: 18px;"
		        + "            margin-bottom: 10px;"
		        + "            text-align: left; /* Left-align the p tag */"
		        + "        }"
		        + "        a {"
...
for (Brute ob1 : allcomb) {
    ...
    for (Map.Entry<String, HashMap<String, List<List<String>>>> subjectEntry : ob1.embedded.entrySet()) {
        ...
        for (Map.Entry<String, List<List<String>>> teacherEntry : subjectEntry.getValue().entrySet()) {
            htmlContent += "<p>" + removeNumbers(teacherEntry.getKey())+": ";
            htmlContent +=teacherEntry.getValue().get(0)+" ";
            htmlContent += teacherEntry.getValue().get(1);
        }
    }
    htmlContent += "</div>";
